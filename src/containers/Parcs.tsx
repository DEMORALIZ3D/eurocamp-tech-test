...
import {
  hasParcsError,
  parcsSelector,
  isParcsLoading as isParcsLoadingSelector,
} from "../store/Parcs/selector";
...
const Parcs = () => {
  ...
  const fetchParcsData = useCallback(async () => {
    ...
    try {
      const result = await axios.get<{ data: ParcResponse }>(
        `http://localhost:3001/api/1/parcs`,
      );
      console.log(result);
      if (result.data) {
    ...
    <div>
      <Box sx={{ pb: 4 }}>
        ...
        <Typography variant="body1" component="p">
          Here you can find a total list of parks
        </Typography>
      </Box>
      {isParcsLoading ? (
        <LoadingCircle />
                ...
                allParcs
                  .filter(
                    (parc) =>
                      ...
                      parc.name.includes(searchValue),
                  )
                  .map((parc) => (
                    <Card
                      key={parc.id}
                    ...
                    >
                      ...
                      <CardMedia
                        ...
                        alt="Paella dish"
                      />
                      <CardContent>{parc.description}</CardContent>
                      <CardActions
                        sx={{
                          display: "flex",
                          alignItems: "flex-end",
